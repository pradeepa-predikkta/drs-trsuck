...
class Autospa_ThemeHelper
{
	...
	static function getPostOption($prefix=null)
	{
		...
		{
			if(preg_match('/^'.AUTOSPA_THEME_OPTION_PREFIX.$prefix.'/',$key,$result)) 
			{
				$index=preg_replace('/^'.AUTOSPA_THEME_OPTION_PREFIX.'_/','',$key);
				$option[$index]=$value;
			}
		}	
		
		$option=Autospa_ThemeHelper::stripslashesPOST($option);
	...
	{
		if(!$display) return(AUTOSPA_THEME_OPTION_PREFIX.'_'.$name);
		echo AUTOSPA_THEME_OPTION_PREFIX.'_'.$name;
	}
	...
	{
		return(Autospa_ThemeHelper::displayIf($value,$testValue,' disabled ',$display));
	}
	...
	{
		return(Autospa_ThemeHelper::displayIf($value,$testValue,' checked ',$display));
	}
	...
	{
		return(Autospa_ThemeHelper::displayIf($value,$testValue,' selected ',$display));
	}
	...
	static function createTermDictionary($term,$arg=array(),$data=array(),$element=array(),$key='slug')
	{
		...
		if(in_array('useNone',$element)) $dictionary[-1]=__('- None -','autospa');
		if(in_array('useSelect',$element)) $dictionary[-1]=__('[Select]','autospa');
		if(in_array('useDefault',$element)) $dictionary[-1]=__('[Use default settings]','autospa');
